...
"""IA-pix2seq data loading and image manipulation utilities."""
...
class SeqLoader(object):
    """ Class for loading data from raw data (sequence only) """
    ...
    def __init__(self,
               ...
               limit=1000):
        self.batch_size = batch_size  # minibatch size
        self.max_seq_length = get_max_len(strokes)  # N_max in sketch-rnn paper
        self.scale_factor = scale_factor  # divide data by this factor
        self.random_scale_factor = random_scale_factor  # data augmentation method
        self.limit = limit  # removes large gaps in the data
        self.augment_stroke_prob = augment_stroke_prob  # data augmentation method
    ...
    def _get_batch_from_indices(self, indices):
        ...
        for i, idx in enumerate(indices):
            ...
            if self.augment_stroke_prob > 0:
                data = augment_strokes(data, self.augment_stroke_prob)
            seq_batch.append(data)
            seq_len_batch.append(len(data))
        
        return self.pad_seq_batch(seq_batch, self.max_seq_length), seq_len_batch


    def random_batch(self):
        """Return a randomised portion of the training data."""
        idxs = np.random.permutation(range(0, len(self.strokes)))[0:self.batch_size]
        return self._get_batch_from_indices(idxs)
...
class SeqLabelLoader(object):
    ...
    def _get_batch_from_indices(self, indices):
        ...
        for i, idx in enumerate(indices):
            ...
            seq_batch.append(data)
            seq_len_batch.append(len(data))
            lab_batch.append(self.labels[idx])
        
        return self.pad_seq_batch(seq_batch, self.max_seq_length), seq_len_batch, lab_batch
    ...
    def _get_batch_from_indices(self, indices):
        ...
        for i, idx in enumerate(indices):
            ...
            if self.augment_stroke_prob > 0:
                data = augment_strokes(data, self.augment_stroke_prob)
            seq_batch.append(data)
            png_batch.append(self.images[idx])
        ...
        png_batch = self.random_rotate_png(png_batch)
        png_batch = self.random_translate_png(png_batch)
        return self.pad_seq_batch(seq_batch, self.max_seq_length), png_batch, label_batch
...
class SeqImgLabelLoader(object):
    """ Class for loading data from raw data (sequence and image) """
    ...
    def idx_sample(self, idx):
        ...
        label = self.labels[idx]
        return strokes_5d, png, len(strokes_5d), label
    ...
    def _get_batch_from_indices(self, indices):
        ...
        png_batch = self.random_rotate_png(png_batch)
        png_batch = self.random_translate_png(png_batch)
        return self.pad_seq_batch(seq_batch, self.max_seq_length), png_batch, len_batch, label_batch
